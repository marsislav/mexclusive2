...
$discussion = mexclusive2_get_discussion_data();
?>
	...
	<div id="comments" class="bg-light rounded p-4 <?php echo comments_open() ? 'comments-area' : 'comments-area comments-closed'; ?>">
		...
		if ( have_comments() ) :
			...
			<ol class="comment-list">
				...
				wp_list_comments(
					array(
						'walker'      => new mexclusive2_Walker_Comment(),
						'avatar_size' => mexclusive2_get_avatar_size(),
						'short_ping'  => true,
						'style'       => 'ol',
					)
				);
			...
			if ( have_comments() ){
				$prev_icon     = mexclusive2_get_icon_svg( 'chevron_left', 22 );
				$next_icon     = mexclusive2_get_icon_svg( 'chevron_right', 22 );
				$comments_text = __( 'Comments', 'mexclusive2' );
				the_comments_navigation(
					array(
						'prev_text' => sprintf( '%s <span class="nav-prev-text"><span class="primary-text">%s</span> <span class="secondary-text">%s</span></span>', $prev_icon, __( 'Previous', 'mexclusive2' ), __( 'Comments', 'mexclusive2' ) ),
						'next_text' => sprintf( '<span class="nav-next-text"><span class="primary-text">%s</span> <span class="secondary-text">%s</span></span> %s', __( 'Next', 'mexclusive2' ), __( 'Comments', 'mexclusive2' ), $next_icon ),
					)
				);
			}
			...
			if ( comments_open() && 'asc' === strtolower( get_option( 'comment_order', 'asc' ) ) ) :
				...
				<div class="comment-form-flex comment-form-wrapper">
					<h2 class="comments-title"><?php _e( 'Leave a comment', 'mexclusive2' ); ?></h2>
					<?php mexclusive2_comment_form( 'asc' ); ?>
				</div>
			<?php
			endif;
			...
			if ( ! comments_open() ) :
				?>
				<p class="no-comments">
					<?php _e( 'Comments are closed.', 'mexclusive2' ); ?>
				</p>
			<?php
			endif;
...
function mexclusive2_get_discussion_data() {
...
function mexclusive2_comment_form( $order ) {
...
class mexclusive2_Walker_Comment extends Walker_Comment {
		...
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
			<footer class="comment-meta">
				<div class="comment-author vcard">
					 ...
					 */
					if ( mexclusive2_is_comment_by_post_author( $comment ) ) {
						printf( '<span class="post-author-badge" aria-hidden="true">%s</span>', mexclusive2_get_icon_svg( 'check', 24 ) );
					}

					printf(
						wp_kses(
						/* translators: %s: Comment author link. */
							__( '%s <span class="screen-reader-text says">says:</span>', 'mexclusive2' ),
							array(
								'span' => array(
									'class' => array(),
								),
				...
				<div class="comment-metadata">
					...
					$comment_timestamp = sprintf( __( '%1$s at %2$s', 'mexclusive2' ), get_comment_date( '', $comment ), get_comment_time() );
					...
					$edit_comment_icon = mexclusive2_get_icon_svg( 'edit', 16 );
					edit_comment_link( __( 'Edit', 'mexclusive2' ), ' <span class="edit-link-sep">&mdash;</span> <span class="edit-link">' . $edit_comment_icon, '</span>' );
					?>
				...
				if ( $commenter['comment_author_email'] ) {
					$moderation_note = __( 'Your comment is awaiting moderation.', 'mexclusive2' );
				} else {
					$moderation_note = __( 'Your comment is awaiting moderation. This is a preview; your comment will be visible after it has been approved.', 'mexclusive2' );
				}
				?>
...
function mexclusive2_get_avatar_size() {
	return 100;
}


function mexclusive2_is_comment_by_post_author( $comment = null ) {
...
function mexclusive2_get_icon_svg( $icon, $size = 24 ) {
	return mexclusive2_SVG_Icons::get_svg( 'ui', $icon, $size );
}


class mexclusive2_SVG_Icons {
